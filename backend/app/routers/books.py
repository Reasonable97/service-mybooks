...
def read_books(page: int = 1, size: int = 10, db: Session = Depends(get_db)):
    """
    Получить список всех книг с пагинацией (упрощённый формат)
...
def read_book(book_id: int, db: Session = Depends(get_db)):
    """Получить книгу по ID (полный формат со всеми деталями)"""
...
@router.post("/", response_model=schemas.BookBrief, status_code=status.HTTP_201_CREATED)
def create_book(book: schemas.BookCreate, db: Session = Depends(get_db)):
    """Создать новую книгу (возвращает упрощённый формат)"""
    return crud.create_book(db=db, book=book)


@router.patch("/{book_id}", response_model=schemas.BookBrief)
def update_book(book_id: int, book: schemas.BookUpdate, db: Session = Depends(get_db)):
    """Частично обновить книгу (возвращает упрощённый формат)"""
